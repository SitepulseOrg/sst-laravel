import { SecretsManagerClient, GetSecretValueCommand, PutSecretValueCommand, CreateSecretCommand, DeleteSecretCommand, ResourceNotFoundException, DescribeSecretCommand, ListSecretsCommand, } from '@aws-sdk/client-secrets-manager';
 ...
 */
export function getSecretPath(appName, stage) {
    return `/${appName}/${stage}/env`;
}
/**
 * List available stages for an app by inspecting Secrets Manager paths.
 */
export async function listAvailableStages(appName, region) {
 ...
 */
function getChunkPath(basePath, chunkIndex) {
 ...
 */
export function toEnvFileContent(vars) {
 ...
 */
export function needsChunking(vars) {
    return getJsonByteSize(vars) > MAX_SECRET_SIZE_BYTES;
}
/**
 * Get a secret value from AWS Secrets Manager.
 */
async function getSecretValue(client, secretPath) {
    try {
 ...
 */
export async function pullSecrets(secretPath, region) {
    const client = new SecretsManagerClient({ region });
 ...
 */
async function pullChunkedSecrets(client, basePath, chunkCount) {
    const allVars = {};
    // Fetch all chunks in parallel
    const chunkPromises = Array.from({ length: chunkCount }, (_, i) => getSecretValue(client, getChunkPath(basePath, i + 1)));
 ...
 */
export async function getSecretsFingerprint(secretPath, region) {
 ...
 */
export async function secretExists(secretPath, region) {
    const client = new SecretsManagerClient({ region });
    try {
 ...
 */
async function upsertSecret(client, secretPath, secretValue, description, region) {
 ...
 */
async function deleteSecret(client, secretPath) {
    try {
 ...
 */
async function findExistingChunks(client, basePath) {
    const chunks = [];
    try {
 ...
 */
async function cleanupOldChunks(client, basePath, newChunkCount) {
 ...
 */
export async function pushSecrets(secretPath, vars, region) {
    const client = new SecretsManagerClient({ region });
    if (!needsChunking(vars)) {
        // Small enough for a single secret - use legacy format for simplicity
        await upsertSecret(client, secretPath, JSON.stringify(vars), 'Laravel environment variables', region);
    ...
    await upsertSecret(client, secretPath, JSON.stringify(metadata), 'Laravel environment variables (chunked metadata)', region);
    // Create chunk secrets in parallel
    const chunkPromises = chunks.map((chunk, index) => upsertSecret(client, getChunkPath(secretPath, index + 1), JSON.stringify(chunk), `Laravel environment variables (chunk ${index + 1}/${chunks.length})`, region));
 ...
 */
export async function getSecretInfo(secretPath, region) {
    const client = new SecretsManagerClient({ region });
    const secretValue = await getSecretValue(client, secretPath);
    if (!secretValue) {
        return null;
